...
//
//   No phone yet:   how to connect
//   Phone joined:   motor bars, what the rover is doing, distance,
//                   and the last thing the rover said
...
void drawScreen() {
  ...
  if (wifiPhoneConnected()) {
    ...
    screen.print(F("Phone connected"));
...
unsigned long screenState() {
  unsigned long s = wifiPhoneConnected();
...
void screenTick() {
  ...
  // least 46 ms, and meanwhile the Kypruino does not answer the joystick. A
  // busy bus also makes the WiFi link more likely to mishear a letter.
